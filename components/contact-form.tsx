      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <h3 className="text-xl font-bold mb-2">{lang === "en" ? "Message Sent!" : "تم إرسال الرسالة!"}</h3>
        ...
        <CardContent>
          <form action={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="subject" name="subject" required className={isRtl ? "text-right" : "text-left"} />
            </div>

            <div className="space-y-2">
              <Label htmlFor="message">{dictionary.message}</Label>
              <Textarea id="message" name="message" rows={4} required className={isRtl ? "text-right" : "text-left"} />
            </div>
            ...
            <Button type="submit" className="w-full" disabled={isSubmitting}>
              {isSubmitting ? (lang === "en" ? "Sending..." : "جاري الإرسال...") : dictionary.submit}
            </Button>
          </form>
        </CardContent>
      </Card>
