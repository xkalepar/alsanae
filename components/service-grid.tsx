...
export function ServiceGrid({ services, lang }: ServiceGridProps) {
  ...
  const getIcon = (iconName: string) => {
    ...
    return icons[iconName as keyof typeof icons] || <Building2 className="h-8 w-8 text-primary" />
  }
        ...
        >
          <Card className="h-full flex flex-col">
            <CardContent className={`pt-6 flex-1 ${isRtl ? "text-right" : "text-left"}`}>
              <div className="mb-4 rounded-full bg-primary/10 p-3 w-fit">{getIcon(service.icon)}</div>
            ...
            <CardFooter className={`pt-0 ${isRtl ? "justify-end" : "justify-start"}`}>
              <Button variant="outline">{lang === "en" ? "Learn More" : "اعرف المزيد"}</Button>
            </CardFooter>
          </Card>
        </motion.div>
      ))}
